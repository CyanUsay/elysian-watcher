...
- ✅ 当前初始状态为「邀请已暂停」。
- ✅ **状态从暂停变为开放时**自动推送通知：
  - 第一次发现开放：`🚨 Elysian Horizon开放了！`，并紧接着单独发一条**纯邀请链接**通知（标题就是 `https://discord.gg/elysianhorizon`），方便直接复制
  - 之后只要还开着，**每隔 30 分钟重复提醒一次**：`⚠️ Elysian Horizon仍然开放中`（直到再次关闭为止）
- ✅ **每日心跳**：每隔 24 小时发一条 `✅ Elysian 监控正常运行中`（带当前状态），即使状态没变化也能确认脚本还活着。
- ✅ 使用 **Server酱** 推送到微信。
...
| `HEARTBEAT_INTERVAL_HOURS`| 心跳通知间隔（小时），设 `0` 关闭 | `24`              |
...
**Q：状态一直没变，怎么知道脚本有没有在正常跑？**
- 两种办法：
  1. 打开仓库的 **Actions** 页面，每次运行都会有一条记录，**绿勾**就是成功（这是最权威的方式）。
  2. 看微信里的**每日心跳**通知 `✅ Elysian 监控正常运行中`——每 24 小时一条，收到就说明它还活着。
- 注意：当状态「暂停→暂停」没有变化时，脚本不会提交也不会发开放通知，所以**仓库首页的提交历史不会变化**，这是正常的，别据此判断它没跑。
